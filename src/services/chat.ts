...
import type { ChatRoom, Message, BackendChat, ApiResponse, User } from "../types"
import { mapBackendChatsToChatRooms, mapBackendChatToChatRoom } from "../utils/chatMapper"

// 채팅방 목록 가져오기
export async function getChatRooms(): Promise<ChatRoom[]> {
    try {
        // 백엔드 API 호출
        const response = await fetch("http://localhost:8080/api/chats/list", {
            method: "GET",
            headers: {
        ...
        const data: ApiResponse<BackendChat[]> = await response.json()
        ...
        const chatRooms = mapBackendChatsToChatRooms(data.data, currentUser)
        return chatRooms
    } catch (error) {
...
// 채팅방 생성하기
export async function createChatRoom(title: string, topic: string, description?: string): Promise<ChatRoom> {
    // API 요청: 채팅방 생성
