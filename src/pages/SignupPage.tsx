...
interface SignupPageProps {
    onRegister: (userData: { name: string; email: string; password: string; phone?: string }) => Promise<boolean>
}

const SignupPage = ({ onRegister }: SignupPageProps) => {
    ...
    // 이메일 인증 관련 상태
    const [emailVerificationStatus, setEmailVerificationStatus] = useState<"idle" | "requested" | "verified">("idle")
...
const StrengthIndicator = styled.div<PasswordStrengthProps>`
  height: 100%;
  border-radius: 2px;
...
const EmailInput = styled.input<InputProps>`
  flex: 1;
  padding: 0.875rem 1rem 0.875rem 2.75rem;
  border: none;
  font-size: 1rem;
...
const VerificationCodeInput = styled.input<InputProps>`
  flex: 1;
  padding: 0.875rem 1rem 0.875rem 2.75rem;
  border: none;
  font-size: 1rem;
...
const VerificationMessage = styled.div<VerificationMessageProps>`
  display: flex;
  align-items: center;
  gap: 0.5rem;
  padding: 0.5rem;
...
const PasswordRequirement = styled.div<PasswordRequirementProps>`
  display: flex;
  align-items: center;
  gap: 0.25rem;
